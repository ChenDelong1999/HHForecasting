...
class Joint_encoder(nn.Module):
    def __init__(self, backbone_model, backbone_hidden_size, backbone_num_layers, dropout, input_size,
                 head_model='linear', head_hidden_size=36, head_num_layers=3):
    ...
    def forward(self, raindrop, runoff_history, runoff):
        ...
        info = {'WRITER_KEYS': []}
        raindrop = torch.cat([raindrop, torch.unsqueeze(runoff_history, -1)], dim=2)
        ...
        if self.head_model == 'linear':
            prediction = self.prediction_head(feature)
        else:
            prediction = self.prediction_head(feature.transpose(1,2)).transpose(1,2)
